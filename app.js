...
request({ url: url , json:true}, (error, response) => {
    ...
    console.log(response.body.current.weather_descriptions[0]+ ' . Temperature outsdide is '+ response.body.current.temperature+'f. It feels like '+ response.body.current.feelslike +'f outside')
})
...
request({ url: geocodeURL, json: true }, (error, response) => {
    if(error){
        console.log('Could not connect to geocode api')
